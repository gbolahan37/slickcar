...
def increase(request):
    if request.method == 'POST': 
        ...
        newqty.save()
        messages.success(request, 'qunatity updated')
        return redirect('cart')
...
def pay(request):
    if request.method == 'POST': 
        api_key = 'sk_test_7c357e3e11135c2198ebd667388d0ffa9662d92e' #secret key from paystack
        curl = ' https://api.paystack.co/transaction/initialize' #paystack call url
        cburl = 'http://54.227.69.127/callback' #payment confirmation page
        ...
        total = float(request.POST['total']) * 100 #total amount to be chaerged from customer card
        user = User.objects.get(username = request.user.username) #query the model for customer details
        email = user.email #save cusomer email to send to paystack
        ...
        data = {'reference':ref, 'amount':int(total), 'email':user.email, 'callback_url':cburl, 'order_number':order_no, 'order_number':order_no, 'currency':'NGN'}
...
def search(request):
    if request.method == 'POST':
        ...
        return render(request, 'search.htnl')
